        ...
        : m(make_shared<impl>(l))
{
	...
	uint64_t offset;
        ...
        data = l->load(section_type::info, &size, &offset);
        if (!data)
...
dwarf::get_section(section_type type) const
{
        ...
        const void *data = m->l->load(type, &size, &offset);
        if (!data)
                throw format_error(std::string(elf::section_type_to_name(type))
                                   + " section missing");
        m->sections[type] = std::make_shared<section>(offset, section_type::str, data, size, m->sec_info->ord);
