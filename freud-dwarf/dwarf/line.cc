...
// This uses the "convention" described at:
...
line_table::iterator
line_table::find_address_after_prologue(taddr addr) const
{
        iterator prev = begin(), e = end();
        if (prev == e)
                return prev;
        ...
        for (++it; it != e; prev = it++) {
                if (prev->address <= addr && it->address > addr &&
                    !prev->end_sequence) {
			// It's probably the next one, i.e. it
			if (prev->file == it->file &&
				prev->line == it->line &&
				prev->column == it->column)
                        	return it;
			else
				// No prologue?
				return prev;
