...
struct section
{
	uint64_t sec_offset;
        ...
        section(section_type type, const void *begin,
                ...
                : sec_offset(0), type(type), begin((char*)begin), end((char*)begin + length), 
                  fmt(fmt), ord(ord), addr_size(addr_size) { }

        section(const uint64_t offset, 
		section_type type, const void *begin,
                section_length length, 
                byte_order ord, format fmt = format::unknown,
                unsigned addr_size = 0)
                : sec_offset(offset), type(type), begin((char*)begin), end((char*)begin + length), 
                  fmt(fmt), ord(ord), addr_size(addr_size) { }
...
struct cursor
{
	...
	uint64_t read_4_or_8_bytes_field();
