...
public:
        ...
        virtual const void *load(section_type section, size_t *size_out, uint64_t *offset) = 0;
};
...
public:
         ...
         */
        void read(std::shared_ptr<section> cudata, section_offset off);
...
public:
         ...
         */
        expr_result evaluate(expr_context *ctx, std::string &evaluation_string, const die * parent = 0) const;

        /**
         * Short-hand for evaluate(ctx, {argument}).
         */
        expr_result evaluate(expr_context *ctx, taddr argument, std::string &evaluation_string, const die * parent = 0) const;
         ...
         * bounds error, etc.)
         */
        expr_result evaluate(expr_context *ctx, const std::initializer_list<taddr> &arguments, std::string &evaluation_string, const die * parent = 0) const;
...
struct register_content {
	unsigned int reg;
	int64_t offset;
};

struct cfa_eval_result {
	int regnum;
	unsigned offset;
};
...
public:
         ...
         */
        virtual register_content reg(unsigned regnum)
        {
                throw expr_error("CTX reg not implemented");
        }

	virtual void set_reg(unsigned regnum, unsigned newreg, int64_t offset) {
                throw expr_error("CTX set_reg not implemented");
	}

	virtual void set_reg_reg(unsigned regnum, unsigned newreg) {
                throw expr_error("CTX set_reg_reg not implemented");
    	}

	virtual void set_reg_offset(unsigned regnum, int64_t offset) {
                throw expr_error("CTX set_reg_offset not implemented");
	}

	virtual void add_reg_offset(unsigned regnum, int64_t offset) {
                throw expr_error("CTX add_reg_offset not implemented");
	}

	virtual void set_cfa(unsigned regnum, int64_t offset) {
                throw expr_error("CTX set_cfa not implemented");
    	}

	virtual void set_cfa_reg(unsigned regnum) {
                throw expr_error("CTX set_cfa_reg not implemented");
    	}

	virtual void set_cfa_offset(int64_t off) {
                throw expr_error("CTX set_cfa_offset not implemented");
    	}

	virtual cfa_eval_result get_cfa() {
                throw expr_error("CTX get_cfa not implemented");
	}

	virtual unsigned get_cfa_reg() {
                throw expr_error("CTX get_cfa_reg not implemented");
    	}

	virtual int64_t get_cfa_offset() {
                throw expr_error("CTX get_cfa_offset not implemented");
    	}
...
public:
         ...
         * bounds error, etc.)
         */
        expr_result evaluate(expr_context *ctx, const die * parent = 0) const;
...
{
public:
	cfa_eval_result get_cfa(expr_context *ctx) const;
	cfa_eval_result get_cfa_from_eh(expr_context *ctx) const;
        frame(const unit *cu,
             section_offset offset, section_length len);
	static struct cfa_eval_result evaluate_cfa(cursor cur, uint64_t length, expr_context * ctx, uint64_t initial_location);
...
public:
	...
	* TODO: describe me
	*/
        iterator find_address_after_prologue(taddr addr) const;
...
namespace elf
{
        ...
        public:
                ...
                const void *load(section_type section, size_t *size_out, uint64_t *offset)
                {
                        auto sec = f.get_section(section_type_to_name(section), *offset);
                        if (!sec.valid())
                                return nullptr;
                        *size_out = sec.size();
                        return sec.data();
